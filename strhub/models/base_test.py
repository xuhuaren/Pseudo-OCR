...
class BaseSystem(pl.LightningModule, ABC):
    ...
    def _eval_step(self, batch, validation: bool, IV) -> Optional[STEP_OUTPUT]:
        images, labels = batch
        total_iv = 0
        total_oov = 0
        correct_iv = 0
        ...
        for pred, prob, gt in zip(preds, probs, labels):
            ...
            if gt in IV:
                ned_iv += edit_distance(pred, gt) / max(len(pred), len(gt))
                if pred == gt:
                    correct_iv += 1
                total_iv += 1
                print("iv")
            else:
                ned_oov += edit_distance(pred, gt) / max(len(pred), len(gt))
                if pred == gt:
                    correct_oov += 1
                total_oov += 1
                print("oov")
        ...
        total_iv = max(total_iv, 0.01)
        total_oov = max(total_oov, 0.01)
        return dict(output=BatchResult(total_iv, correct_iv, ned_iv, total_oov, correct_oov, ned_oov, confidence, label_length, loss, loss_numel))
    ...
    def test_step(self, batch, batch_idx, IV) -> Optional[STEP_OUTPUT]:
        return self._eval_step(batch, False, IV)
