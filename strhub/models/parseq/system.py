...
class PARSeq(CrossEntropySystem):
    ...
    def __init__(self, charset_train: str, charset_test: str, max_label_length: int,
        ...
        ########################################################################
        #replace the fc with ArcFace
        self.head = Arcface(embed_dim, len(self.tokenizer) - 2)
        #print(self.pad_id)
        #print(self.bos_id)
        #assert(0)
        ########################################################################
    ...
    def forward(self, images: Tensor, max_length: Optional[int] = None, labels=None) -> Tensor:
        ...
        if self.decode_ar:
            ...
            if labels is None:
                ...
                logits = self.head(tgt_out, labels)
            ##################################################################
    ...
    def training_step(self, batch, batch_idx) -> STEP_OUTPUT:
        ...
        for i, perm in enumerate(tgt_perms):
            ...
            logits = self.head(out, tgt_out.clone())
            # logits = self.head(out).flatten(end_dim=1)
            ###########################################################
            #print(logits)
            #print(tgt_out.flatten().shape)
